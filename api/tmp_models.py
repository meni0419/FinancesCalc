...
class AttachmentsBinding(models.Model):
    ...
    object_type = models.CharField(max_length=50, db_comment='Тип объекта, к которому привязано вложение (indicator | indicator_to_mo | indicator_to_mo_fact etc.)')
...
class ClosedPeriodValues(models.Model):
    ...
    existence_flag = models.IntegerField(blank=True, null=True, db_comment='Признак наличия записи с сохраненными данными для данного показателя (используется в качестве признака существования сохраненных значений, поскольку некоторые могут иметь null и через значение проверить не получится)')
...
class Indicator(models.Model):
    ...
    calculating_period = models.PositiveIntegerField(db_comment='Indicator calculating period (4 - month, 5 - quarter, 6 - year)')
    ...
    used_supertags = models.TextField(blank=True, null=True, db_comment='Перечень меток, используемых для расчета показателя (суперметки, массив объектов {tag_id: number, value: any})')
    required_supertags = models.TextField(blank=True, null=True, db_comment='Перечень обязательных меток для добавляемых фактов (суперметки)')
    fact_default = models.DecimalField(max_digits=16, decimal_places=4, db_comment='Default fact value')
    auto_create_default_fact = models.IntegerField(db_comment='Whether to automatically create a default fact on expected dates')
...
class IndicatorToMo(models.Model):
    ...
    precision = models.IntegerField(db_comment='Точность округления значений показателя')
    hidden = models.IntegerField(db_comment='Скрытие показателя от всех кроме администраторов и сотрудников которые ответственные за ввод плана\\факта')
    required_supertags = models.TextField(blank=True, null=True, db_comment='Перечень обязательных меток для добавляемых фактов (суперметки)')
...
class IndicatorToMoFact(models.Model):
    ...
    true_indicator_to_mo_id = models.PositiveIntegerField(db_comment='Истинный indicatorToMoId показателя, для которого был внесен факт (без учета признака common)')
    ...
    value = models.DecimalField(max_digits=16, decimal_places=4, blank=True, null=True, db_comment='Значение факт или плана')
...
class IndicatorToMoHst(models.Model):
    ...
    since = models.DateField()
    mark_criterion = models.TextField(blank=True, null=True)
    dependencies = models.TextField(blank=True, null=True, db_comment='Поле хранит информацию о всех зависимостях (indicator_to_mo_id) исходя из формул (вес/план/факт/результат) с полной глубиной (включая зависимости зависимостей)')
    calculating_period = models.PositiveIntegerField(db_comment='Indicator calculating period (4 - month, 5 - quarter, 6 - year)')
    used_supertags = models.TextField(blank=True, null=True, db_comment='Перечень меток, используемых для расчета показателя (суперметки, массив объектов {tag_id: number, value: any})')
    auto_supertags = models.TextField(db_collation='utf8mb4_bin', blank=True, null=True, db_comment='Id меток для автоматического создания при сохранении каждого нового факта')
    fact_default = models.DecimalField(max_digits=16, decimal_places=4, db_comment='Default fact value')
...
class Mo(models.Model):
    ...
    auto_supertags = models.TextField(db_collation='utf8mb4_bin', blank=True, null=True, db_comment='Данные меток для автоматического добавления для каждого нового факта, добавляемого для показателя, которым владеет ОУ')
...
class Report(models.Model):
    ...
    columns = models.TextField(blank=True, null=True, db_comment='Перечень отображаемых по умолчанию колонок в отчете в формате JSON (объект ReportColumn)')
    template_id = models.PositiveIntegerField(db_comment='ID шаблона из report_templates')
    indicator_behaviour_id = models.TextField(blank=True, null=True, db_comment='Comma-separated list of indicator behaviour ids to include in the report')
    indicator_id = models.TextField(blank=True, null=True, db_comment='Comma-separated list of indicator ids to include in the report')
    mo_id = models.TextField(blank=True, null=True, db_comment='Comma-separated list of mo ids to include in the report')
    description = models.TextField(blank=True, null=True, db_comment='Описание отчета')
    tag_id = models.TextField(blank=True, null=True, db_comment='Список id тегов, по которым будет производится выборка данных')
    weighted_only = models.IntegerField(db_comment='Учитывать ли вес показателей при подготовке данных для отчета. Если true, то будут выведены только показатели с весом > 0')
    tags_condition = models.PositiveIntegerField(db_comment='Tags select conditions (true - OR, false - AND)')
    period_start = models.DateField(blank=True, null=True)
    period_end = models.DateField(blank=True, null=True)
    use_own_period = models.IntegerField(db_comment='Использовать период из параметров отчета (true) или текущее значение периода в приложении (false, по умолчанию)')
    cols_sort = models.CharField(max_length=100, blank=True, null=True, db_comment='Тип сортировки показателей в столбцах (id | name | type etc.)')
...
class Subscriptions(models.Model):
    object_type = models.CharField(max_length=50, db_comment='Тип объекта подписки (indicator_to_mo | indicator | mo etc.)')
...
class Supertags(models.Model):
    ...
    values = models.TextField(blank=True, null=True, db_comment='Доступные значения метки в формате json, в виде массива объектов {title: string, value: any}')
    values_source = models.PositiveIntegerField(db_comment='Тип источника значений (0 - статический массив значений в поле values, 1 и более динамические)')
...
class User(models.Model):
    ...
    g_token = models.TextField(blank=True, null=True)
    ...
    emp_code = models.CharField(unique=True, max_length=15, blank=True, null=True, db_comment='Табельный номер сотрудника')
    photo = models.CharField(max_length=255, blank=True, null=True)
    hash = models.CharField(max_length=255)
    sms_phone = models.PositiveBigIntegerField(blank=True, null=True)
    live_start = models.DateField(db_comment='Дата создания')
    live_end = models.DateField(db_comment='Дата удаления')
    birthday = models.DateField(blank=True, null=True, db_comment='Дата рождения')
    google = models.PositiveIntegerField()
    ...
    blocked = models.PositiveIntegerField(db_comment='Признак блокировки пользователя (0/1)')
    description = models.TextField(blank=True, null=True)
    onesignal_id = models.TextField(blank=True, null=True, db_comment='OneSignal Player ID(s)')
    layout = models.ForeignKey(Layouts, models.DO_NOTHING, blank=True, null=True, db_comment='Layout id (see layouts)')
